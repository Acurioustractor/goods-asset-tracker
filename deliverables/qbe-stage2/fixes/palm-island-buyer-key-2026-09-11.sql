...
--   All 17 rows in public.goods_procurement_entities linked to it are Cape York and Cooktown
--   organisations (Cooktown Baptist Fellowship, Vera Scarth-Johnson Gallery, and so on).
--   public.gs_entities holds 21 real Palm Island entities under lga_name = 'Palm Island', all
--   postcode 4816.
--
-- The wider state of the table, which this script does NOT fix:
--   4,551 links total. 3,255 have an entity_id that does not resolve to public.gs_entities at all.
...
--
-- Every table is schema-qualified. If step 0 does not report database `postgres` with 4,551 links,
-- the editor is pointed at the wrong Supabase project. The right one is tednluwflfhxyucgwigh,
-- named "JusticeHub + GrantScope".
