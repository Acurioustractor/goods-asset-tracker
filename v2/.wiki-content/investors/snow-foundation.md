# Snow Foundation

> Existing anchor funder. $193K received across multiple rounds. Long-term partnership model (not one-off grants). Co-invested in Jigsaw alongside the SEFA-led syndicate. Sally Grimsley-Ballard sits on Goods advisory group.

## Mandate

- Long-term enterprise partnership model.
- Flexible capital over time (grants, multi-year).
- Fellowship model for founders.
- Co-investment alongside other catalytic capital.

Rebecca Parkinson (QBE induction): "Snow Foundation partners long-term, flexible capital over time, fellowship model."

## Relationship

- **Sally Grimsley-Ballard** sits on Goods advisory group.
- Primary anchor funder, relationship 2+ years.
- "Final Snow pitch with Snow feedback and reflections" archived at `v2/docs/Final Snow pitch with Snow feedback and reflections.md`.

## Current state

- Received: $193K cumulative.
- Pipeline: Snow R4 at $200K target.
- Proposed 2026 role: **letter of support** acting as catalytic signal to open SEFA + Mindaroo + PFI commitments.

## Playbook

1. Ask for a letter of support (Snow's name on paper opens the rest of the stack).
2. Open the R4 proposal conversation.
3. Reference Jigsaw co-investment pattern when proposing syndicate role.

## Alignment score

See [[alignment-tool]]. Perfect or near-perfect across every dimension. Partnership-grade.

## Related

- [[../capital/capital-stack]]
- [[../capital/australian-case-studies]] (Jigsaw co-investor)
- [[../support-network/advisory-group]]

## Sources

- `v2/docs/Final Snow pitch with Snow feedback and reflections.md`
- `thoughts/shared/qbe-program/session-1-learnings.md`
