...
/**
 * THE PITCH, shared between two doors. `variant: 'qbe'` is the full version built for the QBE
 * application, with the raise broken out by funder and the specific request. `variant: 'overview'`
 * is the same story and the same model without the QBE-specific ask, for anyone else it is shown
 * to. Never invert this: the overview drops detail, it never adds a claim the QBE version doesn't
 * make.
 */
    ...
    <main className="bg-goods-cream text-goods-ink">
      <nav className="sticky top-16 z-20 border-b border-[#e6dfd1] bg-goods-cream/90 backdrop-blur" aria-label="Sections">
        <div className="mx-auto flex max-w-6xl items-center gap-4 overflow-x-auto px-6 py-3 text-sm [scrollbar-width:none] md:px-10 lg:px-14 [&::-webkit-scrollbar]:hidden">
          ...
          <span className="hidden whitespace-nowrap text-[#7a7363] xl:inline">{isQbe ? 'The pitch, for QBE' : 'The pitch'}</span>
      ...
      <header id="crux" className="relative scroll-mt-14 overflow-hidden bg-goods-ink text-goods-cream">
        ...
        <div className="relative mx-auto flex min-h-[86vh] max-w-6xl flex-col justify-end px-6 pb-16 pt-32 md:px-10 lg:px-14">
          <p className="mb-4 text-sm text-goods-cream/70">{isQbe ? 'The pitch, for QBE' : 'The pitch'} · {STORY_UPDATED}</p>
      ...
      <Section id="money" number={11} title={money.title} dark>
        ...
        {!isQbe && (
          <p className="mt-14 max-w-3xl text-goods-cream/80">
            What is real today: {PAID_BEDS} beds bought and paid for on invoice by four organisations. The buyers are named by type: a national research centre, a foundation buying for the Utopia homelands, a homeland school company and a health service. A raise is in progress with several funders, each with a distinct job; the specific figures sit in the funder-facing version of this pitch.
          </p>
        )}
      </Section>
      ...
      <Section id="questions" number={14} title={questions.title} tight>
        ...
        {isQbe ? (
          ...
          <p className="mt-14 text-sm text-[#7a7363]">Words {STORY_UPDATED}. Figures from the register and canon.</p>
        )}
      </Section>
    </main>
  );
