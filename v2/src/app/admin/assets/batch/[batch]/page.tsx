...
export default async function BatchDetailPage({
  ...
  if (assetsRes.error) {
    return (
      <div className="p-6">
        <h1 className="text-xl font-bold">Batch {batch}</h1>
  ...
  return (
    <div className="space-y-6 pb-16">
      <header className="space-y-2">
        ...
        <h1 className="text-2xl font-bold tracking-tight">Batch {prefix}</h1>
        ...
        <div className="flex flex-wrap gap-2">
          {[...statusCounts.entries()].map(([s, n]) => (
            <Badge key={s} className={`text-xs ${STATUS_STYLE[s] || 'bg-gray-100 text-gray-700'}`}>
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          <a
            ...
            className="rounded-md border bg-white px-3 py-1.5 font-medium text-gray-700 hover:bg-gray-50"
          >
          ...
          <tbody>
            {assets.map((a) => (
              <tr key={a.unique_id} className="border-b last:border-0 hover:bg-gray-50">
                ...
                <td className="py-1.5 px-3 text-xs text-gray-700">{a.name || '—'}</td>
                <td className="py-1.5 px-3 text-xs text-gray-600">{a.product || '—'}</td>
                <td className="py-1.5 px-3">
                  <Badge className={`text-xs ${STATUS_STYLE[a.status || 'unknown'] || 'bg-gray-100 text-gray-700'}`}>
                    {(a.status || 'unknown').replace(/_/g, ' ')}
                  </Badge>
                </td>
                <td className="py-1.5 px-3 text-xs text-gray-600">{a.community || '—'}</td>
                <td className="py-1.5 px-3 text-xs text-gray-500">{a.supply_date || '—'}</td>
