  ...
  return (
    <figure className="overflow-hidden rounded-md border border-goods-sand bg-white">
      ...
      <figcaption className="p-2">
        <p className="text-xs font-semibold leading-snug text-goods-ink">{photo.label}</p>
      </figcaption>
    </figure>
  );
}
...
function ArcCard({ item, index }: { item: (typeof missionArc)[number]; index: number }) {
  return (
    <article className="rounded-lg border border-goods-sand bg-white p-3">
      ...
      <h3 className="mt-3 text-base font-semibold text-goods-ink">{item.step}</h3>
      <p className="mt-2 text-xs leading-relaxed text-goods-sub">{item.line}</p>
    </article>
  );
}
    ...
    <article className="rounded-md border border-goods-sand bg-white p-3">
      <div className="flex gap-3">
        {person.photo ? (
          ...
          <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-md border border-dashed border-[#d9cdb8] bg-goods-cream-muted text-goods-sub">
            <ImageIcon className="h-4 w-4" />
          </div>
        )}
        <div>
          <h3 className="text-sm font-semibold text-goods-ink">{person.name}</h3>
          <p className="mt-1 text-[11px] leading-relaxed text-goods-sub">{person.location}</p>
        </div>
      </div>
      {person.quote && <p className="mt-3 text-xs leading-relaxed text-goods-ink">&quot;{person.quote}&quot;</p>}
      <p className="mt-3 text-[11px] leading-relaxed text-goods-sub">{person.deckUse}</p>
    </article>
  );
}
...
export default function MiroReadyBoardPage() {
  ...
  return (
    <main className="min-h-screen bg-[#ebe3d5] text-goods-ink">
      <div className="sticky top-0 z-20 border-b border-[#d9cdb8] bg-goods-cream/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1800px] items-center justify-between gap-4 px-6 py-3">
          ...
          <p className="text-xs uppercase tracking-[0.22em] text-goods-sub">Miro-ready board</p>
        </div>
      </div>

      <div className="mx-auto max-w-[1800px] space-y-8 px-6 py-8">
        <section className="rounded-lg bg-goods-ink p-8 text-goods-cream">
          <p className="text-sm uppercase tracking-[0.28em] text-goods-terracotta">Goods on Country</p>
          <h1
          ...
          <p className="mt-6 max-w-4xl text-xl leading-relaxed text-goods-cream/70">{workshopThesis.reason}</p>
        </section>
                ...
                <article key={motif.label} className="rounded-lg border border-goods-sand bg-white p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <h3
                        className="mt-2 text-2xl font-light leading-tight text-goods-ink"
                        style={{ fontFamily: 'var(--font-display, Georgia, serif)' }}
                      >
                        {motif.line}
                      </h3>
                      <p className="mt-3 text-sm leading-relaxed text-goods-sub">{motif.use}</p>
                    </div>
                  </div>
                </article>
              ))}
          ...
          <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
              ...
              {fieldNoteBeats.map((beat) => (
                <article key={beat.title} className="overflow-hidden rounded-lg border border-goods-sand bg-white">
                  ...
                  <div className="p-3">
                    ...
                    <h3 className="mt-2 text-base font-semibold text-goods-ink">{beat.title}</h3>
                    <p className="mt-2 text-xs leading-relaxed text-goods-sub">{beat.line}</p>
                  </div>
                </article>
              ))}
            </div>
            <div className="space-y-3">
              <div className="rounded-lg border border-goods-sand bg-goods-ink p-5 text-goods-cream">
                ...
                <p className="mt-4 text-lg leading-relaxed text-goods-cream/80">
                ...
                <Link
                  ...
                  className="mt-5 inline-flex items-center gap-2 rounded-md bg-goods-cream px-3 py-2 text-sm font-semibold text-goods-ink"
                >
                  Open field note
                  <ExternalLink className="h-4 w-4" />
                </Link>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {messageLenses.map((lens) => (
                <article key={lens.id} className="rounded-lg border border-goods-sand bg-white p-4">
                  ...
                  <p className="text-sm leading-relaxed text-goods-ink">{lens.line}</p>
                  <p className="mt-3 text-[11px] leading-relaxed text-goods-sub">{lens.watchOut}</p>
                </article>
              ))}
            </div>
          </BoardFrame>
          ...
          <div className="grid gap-5 xl:grid-cols-3">
            {deckPhotoSlots.map((slot) => (
              <article key={slot.id} className="rounded-lg border border-goods-sand bg-goods-cream p-4">
                ...
                <p className="mt-3 text-[11px] leading-relaxed text-goods-sub">{slot.check}</p>
              </article>
            ))}
          </div>
        </BoardFrame>
              ...
              {themeThreads.map((thread) => (
                <article key={thread.id} className="rounded-lg border border-goods-sand bg-white p-4">
                  ...
                  <blockquote className="mt-3 rounded-md bg-goods-cream-muted p-3 text-sm leading-relaxed">
                    &quot;{thread.bestQuote}&quot;
                    <footer className="mt-2 text-[11px] font-semibold text-goods-sub">{thread.quoteBy}</footer>
                  </blockquote>
                </article>
              ))}
            </div>
              ...
              {decisionLanes.map((lane) => (
                <article key={lane.label} className="rounded-lg border border-goods-sand bg-goods-cream p-4">
                  ...
                  <ul className="mt-4 space-y-3">
                    {lane.items.map((item) => (
                      <li key={item} className="flex gap-2 text-sm leading-relaxed text-goods-ink">
                        <CheckCircle2 className="mt-0.5 h-4 w-4 flex-shrink-0 text-goods-sage" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <ul className="mt-4 space-y-3">
                {deckDecisionPrompts.map((prompt) => (
                  <li key={prompt} className="flex gap-2 text-sm leading-relaxed text-goods-ink">
                    <MessageSquareQuote className="mt-0.5 h-4 w-4 flex-shrink-0 text-goods-terracotta" />
                    <span>{prompt}</span>
                  </li>
                ))}
              </ul>
