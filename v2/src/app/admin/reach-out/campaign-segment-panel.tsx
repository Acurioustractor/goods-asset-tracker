  ...
  return (
    <Card>
      <CardContent className="space-y-5">
        ...
        {loading ? (
          <p className="text-sm text-muted-foreground">Resolving live size from GHL…</p>
        ) : error ? (
          ...
          <div className="rounded border border-border bg-muted p-3 text-sm">
            ...
            {preview.enabled ? (
              <div className="flex flex-wrap items-baseline gap-x-6 gap-y-1">
                <span>
                  <strong className="text-lg text-foreground">{count}</strong> {unit}
                </span>
                {typeof preview.withEmail === 'number' && (
                  <span className="text-muted-foreground">
                    <strong className="text-foreground">{preview.withEmail}</strong> with an email
                  </span>
                )}
                ...
                <ul className="mt-2 space-y-1 text-muted-foreground">
                  {preview.sample.map((s, i) => (
                    <li key={i} className="flex flex-wrap items-center gap-2">
                      ...
                      {s.email && <span className="font-mono text-muted-foreground">{s.email}</span>}
        ...
        <div className="rounded border border-border p-3">
          <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
            <Megaphone className="h-4 w-4 text-muted-foreground" /> Build this smart list in GHL
          </div>
          <p className="mt-1 text-sm text-foreground">{segment.ghlSmartListRecipe}</p>
          {tag && (
            <button
              onClick={copyTag}
              className="mt-2 inline-flex items-center gap-1.5 rounded border border-input bg-card px-2 py-1 font-mono text-xs text-foreground hover:bg-muted"
            >
        ...
        <Link
          href={reportHref}
          className="flex items-start gap-3 rounded border border-accent/30 bg-accent/10 p-3 transition-colors hover:bg-accent/20"
        >
          <FileText className="mt-0.5 h-4 w-4 shrink-0 text-accent" />
          <div>
            <div className="text-sm font-semibold text-accent-foreground">Recommended report: {reportName}</div>
            <p className="mt-0.5 text-xs text-accent-foreground">{segment.campaignNote}</p>
          </div>
        </Link>

        {/* No in-app send */}
        <div className="flex items-start gap-2 rounded border border-border bg-muted p-3 text-xs text-muted-foreground">
          <Mail className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
          <span>
            <strong className="text-foreground">GHL owns the send.</strong> This tool only defines the
