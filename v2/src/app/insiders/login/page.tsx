...
function LoginForm() {
  ...
  const from = params.get('from') || '/insiders'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/insiders/auth', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-stone-50 flex items-center justify-center px-6">
      <div className="w-full max-w-md">
        ...
        <p className="mt-3 text-sm text-stone-600 leading-relaxed">
          This area is shared with advisors, funders, and program partners of Goods on
          Country. If you don't have the password, contact Ben at ben@benjamink.com.au.
        </p>
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          <div>
            <label htmlFor="pw" className="block text-xs font-semibold uppercase tracking-wider text-stone-500">
              Password
            </label>
            <input
              id="pw"
...
export default function InsidersLoginPage() {
  return (
