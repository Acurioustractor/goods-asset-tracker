/**
 * The S15 money-flow diagram as one SVG, drawn from the typed data in
 * @/lib/data/money-flow. Same method as the structure sheet (structure-svg.ts):
 * cells on a grid, arrows computed from cell edges, labels that find a clear
 * spot. The helpers are a copy rather than an import because each sheet keeps
 * its own placement report.
 */

import {
  FLOW_ENTITIES as ENTITIES,
  FLOW_LINKS as LINKS,
  FLOW_SHEET as SHEET,
  FLOW_H as STRUCTURE_H,
  FLOW_READ_AT as STRUCTURE_READ_AT,
  FLOW_W as STRUCTURE_W,
  type FlowEntityId as EntityId,
  type FlowEntityStyle as EntityStyle,
  type FlowLink as Link,
  type FlowLinkKind as LinkKind,
  type Side,
} from '@/lib/data/money-flow';
...
const LINE: Record<LinkKind, { stroke: string; width: number; dash?: string; marker: boolean }> = {
  money: { stroke: COLOUR.green, width: 4, marker: true },
  goods: { stroke: COLOUR.terracotta, width: 4, marker: true },
...
export const LAYOUT = {
  ...
  rows: 12,
  gutter: 8,
  /** Columns 6 to 7 and 15 to 16 are arrow channels; rows 5, 6 and 10 carry the vertical arrows. */
  cells: {
    philanthropy: { col: 1, colSpan: 5, row: 1, rowSpan: 4 },
    sefa: { col: 1, colSpan: 5, row: 5, rowSpan: 4 },
    qbe: { col: 1, colSpan: 5, row: 10, rowSpan: 3 },
    goods: { col: 8, colSpan: 7, row: 1, rowSpan: 8 },
    facilities: { col: 8, colSpan: 7, row: 10, rowSpan: 3 },
    orgs: { col: 17, colSpan: 7, row: 1, rowSpan: 5 },
    customers: { col: 17, colSpan: 7, row: 8, rowSpan: 5 },
...
function entity(id: EntityId): string {
  ...
  const baseTitle = big ? 32 : 21;
  const baseLine = big ? 18 : 15;
  ...
  for (const scale of [1, 0.95, 0.9, 0.85, 0.8, 0.75]) {
    ...
    const amountH = e.amount ? titleSize * 1.5 + 4 : 0;
    const total = (titleLines.length - 1) * titleSize * 1.15 + titleSize + amountH + 6 + (bodyLines.length - 1) * lineSize * 1.3 + lineSize;
  ...
  const amountH = e.amount ? titleSize * 1.5 + 4 : 0;
  const a = e.amount ? textBlock([e.amount], textX, top + (titleLines.length - 1) * titleLh + titleSize * 1.5, { size: titleSize * 1.4, face: 'display', weight: 700, fill: COLOUR.green }, owner) : { svg: '' };
  const l = textBlock(bodyLines, textX, top + (titleLines.length - 1) * titleLh + amountH + gap + lineSize, { size: lineSize, face: 'body', fill: e.style === 'dormant' ? COLOUR.ink3 : COLOUR.ink2, lineHeight: lineLh }, owner);
  return `<g data-entity="${id}">${card(r, st, owner)}${t.svg}${a.svg}${l.svg}</g>`;
}
...
function placeLabel(link: Link, idx: number, curve: ReturnType<typeof linkPath>): string {
  ...
  const lines = wrap(link.label, link.labelWidth ?? 200, size, 'body');
  const lh = size * 1.2;
...
function links(): string {
  const markers = (Object.keys(LINE) as LinkKind[])
    ...
    .map((k) => `<marker id="m-arrow-${k}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="4" markerHeight="4" orient="auto-start-reverse" markerUnits="strokeWidth"><path d="M1 1 L9 5 L1 9 Z" fill="${LINE[k].stroke}"/></marker>`)
    .join('');
  ...
  const paths = LINKS.map((l, i) => {
    ...
    return `<path d="${c.d}" fill="none" stroke="${st.stroke}" stroke-width="${st.width}"${st.dash ? ` stroke-dasharray="${st.dash}"` : ''} stroke-linecap="round"${st.marker ? ` marker-end="url(#m-arrow-${l.kind})"` : ''}/>${cross}${label}`;
  }).join('');
...
function footer(): string {
  ...
  const lines = wrap(SHEET.footer, STRUCTURE_W - margin * 2, 13, 'body');
  const t = textBlock(lines, margin, STRUCTURE_H - margin - footerH + 14, { size: 13, face: 'body', fill: COLOUR.ink3, lineHeight: 16 }, 'footer');
...
export function renderMoneyFlowSvg(opts: RenderOptions = {}): string {
  report = [];
  const xmlns = opts.standalone === false ? '' : ' xmlns="http://www.w3.org/2000/svg"';
  const order: EntityId[] = ['philanthropy', 'sefa', 'qbe', 'goods', 'facilities', 'orgs', 'customers'];
  const body = [
  ...
  return `<svg${xmlns} viewBox="0 0 ${STRUCTURE_W} ${STRUCTURE_H}" width="${STRUCTURE_W}" height="${STRUCTURE_H}" role="img" aria-labelledby="flow-title" data-money-flow="svg" data-read-at="${STRUCTURE_READ_AT}">
<title id="flow-title">Goods on Country, where each dollar goes</title>
${body}
</svg>`;
}

export function layoutReport(opts: RenderOptions = {}): Placed[] {
  renderMoneyFlowSvg(opts);
