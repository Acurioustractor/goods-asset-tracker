/**
 * THE MODEL DIAGRAMS, drawn from the modules, for both surfaces.
 *
 ...
 *
 * Every drawing takes the audience. On the public surface no foundation or lender is named, no
 * form question or file is cited, the sole trader's ABN is not printed, and the calendar does not
 * exist. On the working copy the same drawing carries the lines by name.
 *
 * To add a drawing: write a function of the audience that composes kit primitives from module
 * data, add it to QBE_DIAGRAMS with an id, title and the chapter it belongs to. The guards render
 * every entry for both audiences and assert the figures match the modules.
 */
...
export interface QbeDiagram {
  ...
  caption: string;
  /** The Pencil frame it feeds. Working copy only. */
  slide: string;
  /** `working` drawings exist only on the gated copy. */
  audience: 'all' | 'working';
  svg: (audience: StoryAudience) => string;
}

const statusWord = (s: StackLine['status']) => (s === 'ask-made' ? 'ask made' : s);
const STAMP: Record<StoryAudience, string> = {
  public: 'Goods on Country, the model, September 2026',
  working: 'Goods on Country, QBE Catalysing Impact Stage 2, working copy',
};
const PROGRAM_RANGE = `typically ${aud(150_000)} to ${aud(QBE_ASK.full.aud)}`;

// ---------------------------------------------------------------------------
// 01 The entity, and how the money moves

export function entityAndMoney(audience: StoryAudience): string {
  ...
  if (working) {
    ...
    b += panel(MX, ry, 400, rh, { k: 'Where the first beds were made', t: 'The founders, 2024 to 2026', b: 'Beds designed in community, pressed at the farm, delivered to eleven communities. The record the model is built on.', fill: C.sand, stroke: C.sand, tsize: 17, bsize: 13 });
    b += arrow(478, ry + rh / 2, 590, ry + rh / 2);
  }
  ...
  const ins: [string, string, string][] = working
    ? [
    ...
    : [
        ['A catalytic grant', `QBE Catalysing Impact, ${PROGRAM_RANGE}. Every dollar buys beds.`, 'applied'],
        ['Philanthropy', 'Three foundations have asked us to apply. Grants land in the charity.', 'invited'],
        ['Buyers', `ALIVE National Centre paid for ${lineById('alive').beds} beds up front. Centrecorp has a 130-bed quote open.`, 'paid / quote'],
        ['Plant finance', 'Lenders, for equipment and working capital, after the measured run.', 'target'],
      ];
  ins.forEach(([t, d, l], i) => {
    const y = y2 + i * (ih + gap);
  ...
  const holds = [
    'Our board governs purpose, shared assets, appointments and reinvestment. Directors: Kristy Bloomfield, Audrey Deemal, Jeremy Donovan. Indigenous-led. Ownership stays a pathway.',
    'We hold the products, IP, contracts, making, sales, delivery, capital, register and evidence.',
    'We buy the beds, agree the rules with each community, run the measured run, and report.',
    'We repay any equipment debt from our own margin on buyer orders. Never from a community pool. No equity is sold.',
  ];
  let hyy = hy + 92;
  ...
  return frame({
    ...
    sub: working
      ...
      : 'One home for the work, and five community partners who make the local decisions. Every dollar we raise lands in the charity.',
    body: b,
    footer: working
      ? `A Kind Tractor Ltd (ABN 73 669 029 341) is dormant and has no role. The REAL Innovation Fund grant (about $2M, DEWR) is Oonchiumpa's, related and disclosed, not a Goods line. Fallback: ${ENTITY_ROUTE.fallback.applicant} applies, and the answer to Q8 rests on the inter-entity agreement, which is not yet signed.`
      : 'Community partners are independent organisations with their own boards. Ownership stays a pathway wherever it is not legally complete, and no page of ours says otherwise.',
...
export function theUnit(audience: StoryAudience): string {
  ...
  const what: Record<number, string> = working
    ? { 150_000: "one community's pool", [QBE_ASK.smaller.aud]: 'the smaller amount, Q7', [QBE_ASK.recommended.aud]: 'the ask, Q5', [PROGRAM.costAud]: 'the whole program' }
    : { 150_000: "one community's pool", [QBE_ASK.full.aud]: 'the top of the program range', [PROGRAM.costAud]: 'the whole program' };
  const highlight = working ? QBE_ASK.recommended.aud : PROGRAM.costAud;
  let y = ty + 40;
  let x = tx;
  cols.forEach((c, i) => {
  ...
  SCALE_ROWS.forEach((r) => {
    const rh = 74;
    const isHi = r.amountAud === highlight;
    if (isHi) b += box(tx, y + 2, tw, rh - 4, { fill: C.sand, stroke: C.terra, sw: 2, r: 4 });
    ...
    b += text(tx + 10, y + 58, what[r.amountAud] ?? '', { size: 12, font: F.mono, fill: isHi ? C.terra : C.mute, ls: 0.5 });
    y += rh;
    b += line(tx, y, tx + tw, y);
  });
  const py = y + 30;
  ...
  b += para(tx + 20, py + 60, UNLOCK.body.replace("QBE's beds", 'The first beds'), { size: 14.5, width: tw - 40, lh: 1.45 }).svg;

  return frame({
    page: working ? `Form Q5, Q6, Q7. Figures from bed-ratio.ts. The ask buys beds; ${aud(QBE_ASK.recommended.aud)} is ${QBE_ASK.recommended.beds}` : `One bed at ${aud(BED_PRICE_AUD)}. Plastic at 20kg a bed. Local work at about 6.5 modelled hours a bed`,
...
export function theLoop(audience: StoryAudience): string {
  let b = '';
  const y1 = 190, h1 = 112;
  ...
  b += panel(600, y1, 400, h1, { t: 'Our board holds the rules', b: 'Purpose, shared assets, appointments, reinvestment. Indigenous-led today; ownership stays a pathway.', tsize: 17, bsize: 13 });
  ...
  return frame({
    page: audience === 'working' ? 'The model. Figures from community-loop.ts' : 'The model',
    title: 'One catalyst starts five loops a community controls',
    sub: 'The funder acts once. Our board holds the rules. Each community decides what is given, what is sold, who is paid and what the sales money builds next.',
    body: b,
...
export function threeJobs(audience: StoryAudience): string {
  const working = audience === 'working';
  ...
  const foundations = poolLines.filter((l) => l.instrument === 'grant').length;

  const asLine = (l: StackLine) => ({
    name: l.id === 'alive' ? `ALIVE National Centre, ${l.beds} beds` : l.id === 'qbe' ? 'QBE Catalysing Impact, the ask' : l.funder.replace(' Family Foundation, three years', ''),
    ...
    working
      ? [
          ...
          },
          {
            ...
            note: `Cannot be written today: the pressed cost is modelled and the borrower is unsettled. Repaid from our margin on buyer orders, never from a community's pool. At the pressed margin (about ${aud(STAYS_PRESSED_AUD)}, modelled) ${aud(debtTotal)} needs about ${num(bedsPerYearToRepay)} buyer-bought beds a year for three years; at the kit margin (about ${aud(STAYS_KIT_AUD)}) it cannot be repaid.`,
          },
        ]
      : [
          {
            ...
            t: 'Gifts and purchases that buy beds into pools',
            lines: [
              { name: 'A catalytic grant, QBE program', amount: PROGRAM_RANGE.replace('typically ', ''), status: 'applied' },
              { name: `${foundations - 1} foundations that asked us to apply`, amount: 'named amounts', status: 'invited' },
              { name: `ALIVE National Centre, ${alive.beds} beds`, amount: aud(alive.amountAud ?? 0), status: 'paid' },
            ],
            note: `Every dollar here buys beds at ${aud(BED_PRICE_AUD)}. A bought-in kit costs about ${aud(KIT_COST_AUD)} delivered, so the price leaves about ${aud(STAYS_KIT_AUD)} on a bed. Bed money makes beds and nothing else.`,
            lead: true,
          },
          {
            k: 'Money that runs the organisation',
            t: "Three years of support, the organisation's resilience",
            lines: [{ name: 'A three-year invitation to apply', amount: 'three years', status: 'invited' }],
            note: `${THE_BLOCK.line} Bed money never funds this. We raise it separately and we say which is which.`,
          },
          {
            k: 'Money we borrow for the plants',
            t: 'Equipment and working capital, once the cost is measured',
            lines: [{ name: 'The lenders we are talking to', amount: 'after the run', status: 'target' }],
            note: `Cannot be written today: the pressed cost is modelled. Repaid from our margin on buyer orders, never from a community's pool. At the pressed margin (about ${aud(STAYS_PRESSED_AUD)}, modelled) plant finance is repayable from buyer orders; at the kit margin (about ${aud(STAYS_KIT_AUD)}) it is not.`,
          },
        ],
    { y, h },
  );
  ...
  return frame({
    page: working ? 'Form Q14, Q18. Figures from raise-stack.ts' : 'Three kinds of money',
    ...
    body: b,
    footer: working
      ? `${aud(SIGNED_TOTAL_AUD)} is signed today, and it is stated first. ${QBE_ASK.framing} Two lines on Ben's note, FRRR Palm $20K and Luke EV Fleet $20K, have no second source and are not summed.`
      : `${aud(SIGNED_TOTAL_AUD)} is signed today, and we say so first. A catalytic grant is discretionary, ${PROGRAM_RANGE}, from a pool shared across ten enterprises. It sits on top of signed paper and does not double, trigger or guarantee anything.`,
    stamp: STAMP[audience],
  });
}
...
export function whoDecides(audience: StoryAudience): string {
  let b = layers(
    [
      ...
      { k: 'Layer 2, holds', t: 'Goods on Country', d: 'The charity, the brand, the product system, the IP, fundraising, shared services, the register and the evidence. We agree the rules with each community, buy the beds, run the measured run, and report once against one set of numbers.' },
  ...
  b += kicker(rx + 20, 232, 'The test that lets our ownership claim fail');
  ...
  b += para(rx + 20, qy + 10, 'Partial counts as no. Ownership stays a pathway wherever it is not legally complete, and no page of ours says otherwise.', { size: 13, width: rw - 40, lh: 1.4, fill: C.mute }).svg;
  ...
  return frame({
    page: audience === 'working' ? 'Form Q19, Q22. Governance, from the 26 August deck brief and CONTEXT.md' : 'Governance',
    ...
    body: b,
    footer: 'What holds on every page of ours: Indigenous-led today; two Indigenous directors appointed; the aim is full Indigenous directorship; ownership remains a pathway. No Supply Nation, IPP, IBA or First Australians Capital eligibility is inferred from board composition alone.',
    stamp: STAMP[audience],
  });
}

// ---------------------------------------------------------------------------
// 06 What the first grant starts

export function theChain(audience: StoryAudience): string {
  const working = audience === 'working';
  ...
  const alive = lineById('alive');
  const links = [
    { title: 'The first beds go in first', body: working ? `${QBE_ASK.recommended.beds} beds into the first two communities. The first fifty go through our own press and get costed, so the cost of a locally made bed is measured.` : 'Into the first community. The first fifty go through our own press and get costed, so the cost of a locally made bed is measured.', label: working ? `the ask, ${aud(QBE_ASK.recommended.aud)}` : 'the first grant', lead: true },
    ...
    { title: 'Three foundations have already asked us to apply', body: working ? `Tim Fairfax ${aud(tfff.amountAud ?? 0)} over three years (due 9 Oct). Brian M. Davis up to ${aud(bmdf.amountAud ?? 0)} (due 25 Sep). Snow, catch-up booked. Minderoo and Dusseldorp in conversation.` : 'Three written invitations to apply for named amounts, each from a person the funder can call. Their beds make up the rest of the thousand.', label: `${aud(SIGNED_TOTAL_AUD)} signed today` },
    { title: 'Then the plant money', body: working ? `SEFA ${aud(sefa.amountAud ?? 0)} and White Box ${aud(wb.amountAud ?? 0)} for equipment and working capital, once the cost is measured and the borrower is settled.` : 'The lenders we are talking to, for equipment and working capital, once the cost is measured.', label: 'after link 2' },
  ...
  b += band(626, 130, 'The test of catalytic: what remains after the first spend', 'After this money is spent, five communities hold beds, paid assembly and delivery work, sales money where they chose to sell, and the right to decide the next step. The lenders hold a measured cost, so the plants can be financed.');

  return frame({
    page: working ? 'Form Q14, Q18. Leverage is the metric the program publishes: 3.7x in 2025' : 'What a first grant starts',
    title: 'How a first grant is catalytic: a chain, with a condition on every link',
    sub: 'Stated as a chain, never as a total. We never describe a grant as doubling, triggering or guaranteeing anything.',
    body: b,
    footer: working
      ? 'Statuses from raise-stack.ts: invited means a written invitation to apply for a named amount with a callable contact; ask made means an ask is with the funder and no written amount is back; target means our number and nothing from the funder yet; paid means money received and evidenced in Xero.'
      : 'Invited means a written invitation to apply for a named amount with a person the funder can call. Target means our number and nothing from the funder yet. Paid means money received and in the books.',
    stamp: STAMP[audience],
  });
}

// ---------------------------------------------------------------------------
// 07 The calendar (working copy only)

export function theCalendar(audience: StoryAudience): string {
  const b = timeline(
    CALENDAR.map((e, i) => ({
      date: e.date,
      when: e.when,
...
// ---------------------------------------------------------------------------
// 08 The snowball: where it goes if it works

export function theSnowball(audience: StoryAudience): string {
  let b = '';
  b += kicker(MX, 195, 'The four steps, as we say them');
  b += chainRow(
  ...
  const figs: [string, string, string][] = [
    ...
    [SNOWBALL_MARGIN_PER_YEAR_AUD === 1_000_000 ? 'around $1 million a year' : `around ${aud(SNOWBALL_MARGIN_PER_YEAR_AUD)} a year`, `in margin, at about ${aud(SNOWBALL.marginPerBedProseAud)} a bed`, `The model says about ${aud(SNOWBALL.marginPerBedModelAud)} a bed pressed locally. Nobody has measured it. Communities reinvest what they make.`],
  ];
  ...
  return frame({
    page: audience === 'working' ? 'The crux, 3 September 2026. Figures from qbe-story.ts and community-loop.ts' : 'Where it goes if it works. Every figure a target',
...
// ---------------------------------------------------------------------------
// 09 Who buys: the question every room asks
...
export const QBE_DIAGRAMS: readonly QbeDiagram[] = [
  ...
  { id: 'three-jobs', title: 'Capital with three jobs', chapter: 'money', caption: 'Three kinds of money, each with one job, and the measured cost beneath.', slide: '10 (from 08B Lnlxh)', audience: 'all', svg: threeJobs },
  { id: 'the-chain', title: 'How a first grant is catalytic', chapter: 'catalytic', caption: 'What follows, in order, once the first beds go in.', slide: 'Appendix: the chain', audience: 'all', svg: theChain },
  { id: 'the-snowball', title: 'Where it goes if it works', chapter: 'snowball', caption: 'The four steps of the snowball and the direction, said once, as targets.', slide: '09B u09Eoy', audience: 'all', svg: theSnowball },
  { id: 'who-decides', title: 'Who decides what', chapter: 'decides', caption: 'Three layers: our board governs, Goods on Country holds, each community decides. The month-six test beside them.', slide: '07 LhlJr', audience: 'all', svg: whoDecides },
  { id: 'entity-and-money', title: 'The entity, and how the money moves', chapter: 'entity', caption: 'One home for the work, five community partners, and the doors the money comes in by.', slide: '07 LhlJr (footer)', audience: 'all', svg: entityAndMoney },
  { id: 'the-calendar', title: 'The calendar', chapter: 'calendar', caption: 'Three applications inside fourteen days.', slide: 'none yet', audience: 'working', svg: theCalendar },
];

export function diagramsFor(audience: StoryAudience): QbeDiagram[] {
  return QBE_DIAGRAMS.filter((d) => audience === 'working' || d.audience === 'all');
}
