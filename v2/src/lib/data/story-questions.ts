...
export const QUESTIONS: readonly StoryQuestion[] = [
  ...
  },
  {
    ...
    question: `Is ${audRange(RAISE.totalLowAud, RAISE.totalHighAud)} the income?`,
    answer: `No. It is the raise: QBE ${aud(RAISE.qbeAud)} for two production facilities, and ${audRange(RAISE.otherLowAud, RAISE.otherHighAud)} of other philanthropy for the first ${RAISE.bedsYearOne} beds and the support around the trade. Sales money belongs to the community organisation that sells the bed, and none of it is income to Goods on Country.`,
    status: 'answered',
    ...
    since: 'Replaced the $750,000 question.',
  },
  {
    id: 'what-money-buys',
    question: 'What does the money actually buy?',
    answer: `Two things, kept apart. QBE’s ${aud(RAISE.qbeAud)} buys two community production facilities, ${aud(RAISE.qbeAud / RAISE.facilities)} each as a planning allowance; no site has been quoted. The other ${audRange(RAISE.otherLowAud, RAISE.otherHighAud)} buys the first ${RAISE.bedsYearOne} beds at The Harvest Plant, ${RAISE.bedsEach} for each of ${RAISE.communityOrganisations} community organisations. Every bed comes with facilitation and support for the workshopping in community; that sits inside the price of a bed. The beds are the community organisations’ to sell.`,
    status: 'answered',
  ...
  },
  {
    ...
    answer: `About ${aud(BED.makeAud)} on the flat-pack route: tab sheets pressed at The Harvest Plant, leg panels bought in, kits dispatched flat and assembled in community. On a ${aud(BED.priceAud)} bed that leaves ${aud(BED.staysAud)} with the community organisation, with freight paid by the buyer and shown beside the bed, left out of the make cost. The ${aud(BED.makeAud)} is provisional until the bought leg-panel yield is confirmed, and nobody has made fifty in a row at working pace and kept the receipts. That is the measured run.`,
    status: 'partly',
