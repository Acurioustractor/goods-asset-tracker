#!/usr/bin/env node
/**
 * Updates product metadata using Supabase service role key (bypasses RLS)
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=your_key node scripts/fix-products-with-service-key.mjs
 *
...
if (!serviceKey) {
  ...
  console.error('1. Go to https://supabase.com/dashboard/project/cwsyhpiuepvdjtxaozwf/settings/api');
  console.error('2. Copy the "service_role" key (under "Project API keys")');
  console.error('3. Run: SUPABASE_SERVICE_ROLE_KEY=your_key node scripts/fix-products-with-service-key.mjs');
  process.exit(1);
}
...
Like all Goods products, the double Greate Bed is built on a foundation of community ownership. 40% of every sale returns directly to the artisans and their communities.`,
    ...
    metadata: {
      ...
      enterprise_opportunity: {
        enabled: true,
        title: "Run this enterprise in your community",
        description: "License the Goods model for local manufacturing.",
        benefits: ["Full training", "Supply chain", "Marketing", "Quality assurance"],
        contact_cta: "Express Interest"
      }
    }
  },
...
At just 12kg, the Stretch Bed can be carried by one person, yet still supports 200kg and carries the same 5-year warranty as our Greate Bed.`,
    ...
    metadata: {
      ...
      enterprise_opportunity: {
        enabled: true,
        title: "Run this enterprise in your community",
        description: "We provide full weaving technique training.",
        benefits: ["Weaving training", "Supply chain", "Marketing support", "Quality framework"],
        contact_cta: "Express Interest"
      }
    }
  },
  {
    ...
    metadata: {
      ...
      enterprise_opportunity: {
        enabled: true,
        title: "Run this enterprise in your community",
        description: "License the Goods model.",
        benefits: ["Full training", "Supply chain", "Marketing", "Quality assurance"],
        contact_cta: "Express Interest"
      }
    }
...
Specs: 8kg capacity, 50L water per cycle, 1200W solar-compatible, 2-year warranty.`,
    ...
    metadata: {
      ...
      enterprise_opportunity: {
        enabled: true,
        title: "Become a repair hub for your region",
        description: "Communities can become certified repair centres.",
        benefits: ["Repair technician training", "Parts supply chain", "Service contracts", "Ongoing support"],
        contact_cta: "Become a Repair Hub"
      }
    }
  }
];
...
async function updateProducts() {
  console.log('Updating products with metadata...\n');
  ...
  for (const product of products) {
    ...
    if (error) {
      ...
      console.log(`✓ ${data[0].slug} → "${data[0].name}"`);
    } else {
      console.log(`⚠ ${product.slug}: No matching row found`);
    }
  }

  console.log('\nDone! Checking results...\n');

  const { data: all, error: checkError } = await supabase
    .from('products')
  ...
  for (const p of all) {
    ...
    const status = hasMetadata ? '✓ HAS METADATA' : '✗ NO METADATA';
