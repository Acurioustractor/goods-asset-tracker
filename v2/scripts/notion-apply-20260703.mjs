...
await api('PATCH', `/blocks/${DASHBOARD}/children`, { children: [
  ...
  { paragraph: { rich_text:[{ text:{ content:'Signed $0 of $400,000, 8.4 weeks to 31 Aug. Every open Supporter Journey record bucketed (QBE priority / Workbench / Stewarding / Buyer-demand / Parked); the Bucket field now lives on the Funder Pipeline DB. Nine register mirror rows and eight workbench qualify-ins added. Full strategy: wiki/outputs/2026-07-03-pipeline-strategy.md. Share board: Claude Design, Investor Materials, invest-funder-pipeline.' } }] } },
  { paragraph: { rich_text:[{ text:{ content:'Open decisions: SEFA figure ($250K register vs $300K pipeline DB, one figure before the send). Minderoo ask-or-not (Ask made on the register, no formal ask per the pipeline DB). GHL machine token missing contacts/opportunities scopes (machine scripts blind until fixed). Tim Fairfax promotes to the register only on Katie Norman written confirmation. Bryan Foundation stays workbench, nudge first.' } }] } },
] });
