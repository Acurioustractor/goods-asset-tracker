...
The production facility consists of two shipping containers and a diesel generator, designed to be transportable between communities.
...
```
                    ...
                    │  └─────┘ └──────┘ └──────┘ └────┘  │
                    │                                     │
  ┌──────────────┐  │  Router station near entrance       │
  │   20ft       │  │                                     │
  │  CONTAINER   │  └──┬──────────────────────────────────┘
  │  (Shredder)  │     │
  │              │     │  ← Generator sits between containers
  │ ┌──────────┐ │     │    (powers one machine at a time)
  │ │ Zerma    │ │  ┌──┴──┐
  │ │Granulator│ │  │Genr.│
  │ └──────────┘ │  │DG23 │
  │ Shelving     │  └─────┘
...
| Equipment | Make/Model | Location | What It Does |
|-----------|-----------|----------|-------------|
| Granulator (shredder/chipper) | Zerma | 20ft container | Grinds plastic waste into uniform flakes |
| Heat press | Custom "sandwich toaster" | 40ft container | Melts plastic flakes at 180°C into sheets |
| Hydraulic press | Red manual jack, 5000 PSI | 40ft container | Compresses melted sheets to correct thickness |
| Cooling press | Blue frame with jack | 40ft container | Cools sheets under pressure (4 sheets at a time, 2 per layer) |
| CNC router | Carbatec (on bench) | 40ft container | Cuts sheets into flat-pack bed frame parts (3-5 hrs per bed) |
| Edge router | Handheld/bench | 40ft container entrance | Smooths cut edges on finished parts |
| Dust extractor | Carbatec wall-mounted | 40ft container | Captures plastic dust during CNC operation |
| Diesel generator | RW Pacific DG23000SE3 | Between containers | Powers heat press OR chipper (one at a time) |
| Digital scales | Floor-standing | 40ft container | Weighs plastic flake batches |
| Storage bins | Clear plastic tubs | Both containers | Sorted flake storage |
...
Each Stretch Bed requires **~15kg of recycled HDPE/PP plastic waste** (21kg input accounting for waste/offcuts).

### Process Flow

```
COLLECT → SORT → SHRED → WEIGH → FILL TRAY → HEAT → PRESS → COOL → CUT → EDGE → ASSEMBLE
...
**Where:** 20ft container
**Equipment:** Zerma granulator
**Power:** Diesel generator (cannot run heat press at same time)
...
### Step 3: Tray Preparation

**Where:** Prep table, 40ft container
**Equipment:** Scales, metal trays, storage bins
...
**Where:** Heat press, 40ft container
**Equipment:** Custom "sandwich toaster" heat press
**Power:** Diesel generator (cannot run chipper at same time)

1. Switch generator power from chipper to heat press
2. Slide filled tray into heat press
...
**Note:** The pressure gauge (visible on the red hydraulic jack) reads up to 5000 PSI. Target even pressure across the full sheet.

---

### Step 6: Cooling

**Where:** Cooling press, 40ft container
**Equipment:** Blue frame cooling press with jack
...
**Where:** CNC router bench, 40ft container
...
3. Load cutting template on computer
4. Run cutting program
5. Machine cuts all flat-pack parts for one bed (**3-5 hours per sheet**)
6. Vacuum dust between sheets

**Output:** Each sheet produces the flat-pack frame components for one bed.

---

### Step 8: Edge Finishing

**Where:** Router station near 40ft container entrance
...
**Where:** Assembly area, 40ft container
**Equipment:** Cut frame parts + steel poles + canvas

1. Lay out cut recycled plastic parts
2. Slot parts into frame design (no glue, no nails, no tools)
3. Insert 2x galvanised steel poles (26.9mm OD)
4. Stretch heavy-duty canvas sleeping surface over frame
5. **Weight-bearing test:** apply 200kg load to confirm
6. Package flat-pack for delivery
...
| Ear protection | During shredder operation |
...
|---------|---------|
| Emergency | 000 |
| [Site manager] | [Add phone number] |
| [Nearest hospital] | [Add location and number] |
...
6. Start generator if needed

### During Shift

- Log what you're doing as you go (sheets started, sheets completed, issues)
- Take photos of completed sheets
...
- Sheets produced: ___
- Sheets in cooling press: ___
- Plastic shredded (kg): ___

CURRENT STATUS:
- Total sheets completed: ___
- Sheets needed: ___
- Diesel level: ___
...
| Operator | Nic |
| Sheet number | 9 |
| Plastic type | HDPE caps, mixed colour |
| Weight (kg) | 15.2 |
| Heat time (hrs) | 3.5 |
| Press PSI | 5000 |
| Cool time | Overnight |
| Quality (1-5) | 4 |
| Notes | Slightly uneven top-left corner |
| Photo | [attach] |
...
| Metric | Value |
|--------|-------|
| Total sheets produced | 9 (+ 1 cooling) |
| Total plastic recycled (kg) | ~135 |
| Beds ready for assembly | 0 (awaiting CNC) |
| Beds in progress | 0 |
| Target remaining | 6 additional sheets |
...
Goods Production
├── Shift Log (Database)
│   ├── Date, Operator, Shift times
│   ├── Sheets produced, sheets cooling
│   ├── Issues, handover notes
│   └── Photos
├── Sheet Tracker (Database)
│   ├── Sheet number, weight, heat time
│   ├── Quality score, CNC status
│   ├── Status: [Prepped | Heating | Cooling | Cut | Assembled]
│   └── Photo
├── Equipment Log (Database)
│   ├── Equipment name, issue, date
│   ├── Resolution, resolved by
│   └── Maintenance due dates
├── Operations Manual (Page)
│   └── This document, embedded
...
- Scans QR codes on sheets/beds for tracking
- Captures photos inline
- Syncs data when connection is available
- Generates shift reports automatically
- Could integrate with the existing Goods v2 site and Supabase backend

**Tech stack:** Next.js PWA + Supabase (matches existing v2 infrastructure)

This would be worth building when:
- Multiple communities are running production
...
This document should be referenced in the Goods Compendium under a new **Production Knowledge** section:

```markdown
## Production Facility

### Equipment & Process
See: docs/PRODUCTION_FACILITY_GUIDE.md

### Key Facts
- Facility: 2 shipping containers (20ft shredding + 40ft workshop) + diesel generator
- Process: Collect → Sort → Shred → Melt (180°C) → Press (5000 PSI) → Cool → CNC Cut → Edge → Assemble
- Input: 15kg recycled HDPE/PP per bed (21kg accounting for waste)
...
## Handover Checklist (For New Operator Induction)

### Facility & Access
- [ ] Keys/access codes provided for both containers
- [ ] Security arrangements explained (yard access, locks)
- [ ] Power connections demonstrated (generator start/stop)
...
- [ ] Fire extinguisher locations confirmed (both containers)
- [ ] First aid kit location confirmed
- [ ] Emergency contacts provided
- [ ] "No lone working" policy explained
- [ ] PVC identification and exclusion explained
- [ ] Ventilation requirements explained

### Operations Knowledge
- [ ] Full production cycle walked through (video + in-person)
- [ ] Plastic sorting criteria explained and tested
- [ ] Quality check process demonstrated
- [ ] Prep table technique shown (even distribution)

### Documentation & Tracking
- [ ] Notion / tracking system access granted
- [ ] Logging process explained (kg, hours, photos, quality)
- [ ] Shift handover template explained
- [ ] R&D documentation requirements understood
