...
**Updated:** 2026-09-18T06:50:00+10:00
...
**Branch:** MERGED as PR #303 (main 52b47fd, 18 Sep). Worktree `../goods-ledger-wt` is on `snow/after-303`, clean from main. The in-principle letter is the remaining work.
...
[->] **18 September, ~06:50. SHIPPED: PR #303 squash-merged to main at 52b47fd, remote branch deleted, this worktree now on `snow/after-303` (fresh from main). Production verification is the next line in this ledger; if it is missing, the deploy was not yet confirmed live in a browser.** Two traps found on the way: (1) `v2/src/lib/media/exif.ts` was never in git (blanket `media/` ignore) so the local build passed and Vercel failed; (2) after committing it, `.vercelignore` line 17 (`media`) still dropped the folder from the upload, so the file moved to `v2/src/lib/photo-exif.ts`. Never put application code in a folder named media. Six other worktree branches were pushed for safety (carveout, admin-ia, canvas-print-qr, raise-stack, qbe-story, q3-structure); uncommitted work left untouched in goods-lanes (10 files), goods-desk-wt2 (2), goods-placemat-wt (1), goods-public-wt (3).
