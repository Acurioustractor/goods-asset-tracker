# Knockout Criteria

> Six filters every potential investor must pass before we spend time on them. From CASE Smart Impact Capital. Beck Parkinson's discipline: "a quick 'no' is much more helpful than a long drawn-out process."

## The six filters

| # | Criterion | Test | Goods' hard requirement |
|---|---|---|---|
| 1 | **Investment Geography** | Invests in places we operate or are headquartered | Must invest in NT, QLD, WA, SA remote Australia | <!-- brand-lint-ignore-line: remote-australia -->
| 2 | **Legal Structure** | Invests in our legal structure | CLG (Foundation) + Pty Ltd (Ventures); hybrid acceptable |
| 3 | **Investment Size** | Generally invests the amount we are seeking | Tranche-dependent: $100K-$5M range |
| 4 | **Return Expectations** | Aligns on financial / social return | 5-10 year horizon; social-first; community-ownership exit |
| 5 | **Capital Type** | Offers the instrument we need | See [[../capital/capital-stack]], grant, recoverable, sub-debt, senior debt |
| 6 | **Stage of Growth** | Invests at our stage | Early commercial (post-prototype, pre-scale). Not seed, not growth. |

## How we apply this

For every new investor contact:
1. Check each knockout against their stated mandate (website, public deals, referrals).
2. Any single fail = drop or deprioritise.
3. Only pass-all investors advance to the fit criteria in the [[alignment-tool]].

## Who currently passes all six (from our pipeline)

- **QBE Foundation** (program anchor)
- **SEFA** (CDFI, NFP-friendly, right size, debt)
- **Snow Foundation** (existing partner, right structure, right size)
- **IBA** (government Indigenous business fund, right instrument, right geography)
- **PFI (Paul Ramsay Foundation)** (recoverable grant, right size, national mandate)
- **Mindaroo** (catalytic bucket, right instrument; geographic match needs confirmation)
- **First Australians Capital** (CDFI, Indigenous, right instrument)

## Who fails at least one

- **QBE Ventures (CVC):** fails on #2 (equity into CLG structure is complex) and #5 (equity is not 2026 need). Re-evaluate 2027.
- **Generic impact VC funds:** fail on #3 (too early) and #2 (structure).

## Sources

- `raw/2026-03-31-investor-alignment-tool-case.xlsx` (Tab 1 and Tab 3)
- `raw/2026-03-31-qbe-induction-slides-fulltext.md`
- `thoughts/shared/qbe-program/session-1-learnings.md`

## Related

- [[alignment-tool]]
- [[our-investment-needs]]
- [[../capital/capital-stack]]
