#!/usr/bin/env python3
"""Generate individual PNG files for each QR unit with pink elements on WHITE background"""
...
OUTPUT_DIR = BASE_DIR / 'data/new_beds/individual_units_white_bg'
...
WHITE = (255, 255, 255, 255)  # WHITE BACKGROUND
BLACK = (0, 0, 0, 255)  # Black for QR codes
...
print(f"🎨 Generating individual PNG units with WHITE backgrounds...")
...
for i, bed in enumerate(beds, 1):
    ...
    img = Image.new('RGBA', (UNIT_WIDTH, UNIT_HEIGHT), WHITE)
    ...
    # 2. Load QR code and keep as BLACK on WHITE (standard QR code)
    qr_img = Image.open(qr_path).convert('RGBA')
    qr_img = qr_img.resize((QR_SIZE_PX, QR_SIZE_PX), Image.Resampling.LANCZOS)

    # Paste QR code (standard black on white)
    ...
    # Save PNG
    output_path = OUTPUT_DIR / f"{asset_id}_unit_white_bg.png"
    img.save(str(output_path), 'PNG')
    print(f"  ✓ Saved: {output_path.name}")

print()
print(f"✅ Generated {len(beds)} individual unit PNGs with WHITE backgrounds!")
print(f"\n📁 All files saved to: {OUTPUT_DIR}")
print(f"\nEach file is {UNIT_WIDTH}px × {UNIT_HEIGHT}px")
print(f"💗 Pink elements:")
print(f"   - 'Goods' text + ● circle (not square period)")
print(f"   - Asset ID numbers")
print(f"🔲 Black QR codes (standard, scannable)")
print(f"⬜ Background: SOLID WHITE")
print(f"✏️  Font: Helvetica Bold (larger and bolder)")
print(f"\nPerfect for printing services that don't support transparency!")
