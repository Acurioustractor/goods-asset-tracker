...
| Maningrida run actuals (time, diesel, plastic yield). **Highest-value open input**: 250 vs 500 beds/yr is the difference between a site that cannot cover production and one that owns its plant | Ben | Next press run |
| Who pays the line supervisor, and is trainer/WHS ACT's cost or the site's | Ben + Nic | Before the 3-statement model |
...
| Modules, not build paths, in the cost engine | Ben, then Matt | Before an ask is written for Utopia, Tennant Creek or Palm Island |
| Reconcile $110,046 against the MVF's ~$75K | Ben | Before either is quoted again |
