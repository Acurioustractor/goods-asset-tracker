...
| Maningrida run actuals (time, diesel, plastic yield). **Highest-value open input**: 250 vs 500 beds/yr is the difference between a site that cannot cover production and one that owns its plant. **PARKED 2026-07-25.** Not derivable from a desk: the Xero mirror is verified empty and no run records exist in the repo. Needs Ben's own numbers or a live Xero pull. Instrument the next run. **Related and cheaper: `01-vision-and-ambition.md:154` records one sheet per bed against Notion's two sheets and three beds/day. Settling that moves yield and throughput without a measured run** | Ben / Nicholas / Defy | Parked |
...
| ~~Modules, not build paths, in the cost engine~~ **BUILT 2026-07-25** (`capex_modules` + `priceModuleSelection()`, 12 guards; the ladder is untouched). Tennant Creek is now priceable at module level. **Utopia still is not, and the reason is now specific: collection and baling is genuinely unpriced and sits upstream of the shredder.** Palm Island returns $0, recorded as the wrong answer, since governance has a real cost that is not plant. Remaining: a collection quote, and the per-module split of the operating block | Ben, then Matt | Before an ask is written for Utopia or Palm Island |
...
| EL syndication API calling routes that 404 | Ben / EL side | Storyteller pages are on fallback data |
